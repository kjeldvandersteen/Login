...
switch ($request->action) {
    case "createAccount":
        createAccount($conn, $request, $response);
        break;
    case "loginAccount":
        loginAccount($conn, $request, $response);
        break;
    default:
...
function createAccount($conn, $request, $response) {
    ...
    if(isset($request->email) && isset($request->username))
    {
        ...
        // password_hash() op gebruikt is.
    }
    $response->passwordLog = "$hashedpassword";
    die(json_encode($response));
    // Valideer input, check of username of email al bestaan en INSERT de gegeven en het wachtwoord waar
    //query om het toe te voegen aan de data base
}

function loginAccount($conn, $request, $response) {
    // Valideer input, check bij email in tabel of de hash van het wachtwoord overeenkomt
    // met een password_verify() en return de token via de response.
}